...
const articles = [
  ...
  },
  {
    ...
    description: 'Dive into the web where I crafted to assist you in effortlessly managing invoices, quotes, and receipts. This user-friendly platform allows you to send and download these essential documents for free. Whether you are a seasoned user or a newcomer, our documentation offers clear guidance with step-by-step instructions, code examples, and insightful tips to optimize your invoicing process.',
          ...
          <Card>
            <article className='p-6 space-y-6 relative'>
              <header>
                ...
                <span className='text-zinc-400 absolute top-4 right-5'><EyeIcon className='float-right mx-2 my-[2px] w-5 h-5' />{views}</span>
                <h1 className='text-2xl font-semibold my-2'>{article.title}</h1>
                <p className='text-zinc-400 xl:line-clamp-none line-clamp-3'>{article.description}</p>
              </header>
              <aside className='flex space-x-3'>
                ...
                <Link href={article.url} onClick={articleVisited} className='read-more absolute right-2 bottom-5 xl:right-10 xl:bottom-10 flex hover:underline'>
